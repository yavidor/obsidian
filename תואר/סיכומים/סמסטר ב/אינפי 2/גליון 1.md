...
נצייר את המשולש עם זווית $\theta$, צלע "ממול" $x$ וצלע "ליד" $2$
...
# פתרון 2
## סעיף א'
בדומה למה שעשינו באינטגרל העשירי (אבל הפוך) מהשאלה הקודמת, נצייר משולש שזוויתו $\frac{x}{2}$, הבסיס הרחוק יהיה $t$ והבסיס הקרוב יהיה $1$.
...
$$
		...
		= 2\int \frac{1}{\left( t+1 \right)^{2}+2 } \, dx    
		\end{gather}
$$