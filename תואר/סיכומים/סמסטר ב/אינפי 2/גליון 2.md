...
$$
נת\ון כי $M=\sup_{x \in \left[ a.b \right]}\left| f\left( x \right) \right|$, לכן $\forall x \in \left[ a,b \right]:\left| f\left( x \right) \right|\leq M$, על כן $\forall x \in \left[ a,b \right]:\left| f\left( x \right) \right|^{n}\leq M^{n}$, ולכן לכל תת-קטע $\left[ a_{i-1},a_{i} \right]$, $M_{i}\leq M^{n}$
$$
	\begin{gather}
...
$\blacksquare$
## סעיף ב'
יהי $\varepsilon>0$. $\left| f \right|$ רציפה בקטע הסגור $\left[ a,b \right]$, ממשפט ויירשטראס, $f$ חסומה ומקבלת את המקסימום והמינימום שלה בקטע הזה, לכן $M=\max_{x \in \left[ a,b \right]}\left| f\left( x \right) \right|$. 