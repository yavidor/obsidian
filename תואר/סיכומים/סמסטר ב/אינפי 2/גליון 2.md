...
$$
מכיוון ש $f\geq 0$ אבל $f\not\equiv0$, קיימת נקודה כלשהי $x_{0}\in \left[ a,b \right]$ בה $f\left( x_{0} \right)>0$. ממשפט ערך הביניים, קיים $\delta>0$ כך שלכל $x \in \left[ x_{0}-\delta,x_{0}+\delta \right]$ מתקיים $f\left( x \right)\geq \frac{f\left( x_{0} \right)}{2}$, נבחר את $\delta$ כך ש $\delta<\min\left\{ x_{0}-a,b-x_{0} \right\}$ כדי לוודא שנשאר בתחום $\left[ a,b \right]$.
נגדיר חלוקה $P=\left\{a,x_{0}-\delta,x_{0}+\delta,b \right\}$ 

לכל $i\in \left[ \left| P \right| \right]$ מתקיים ש $\Delta_{i}=a_{i}-a_{i-1}> 0$, לכן $m_{k}\Delta_{k}>0$
מכאן שלכל $P$
$$
	U\left( P,f \right) = \sum_{i}m_{i}\Delta_{i}= \underbrace{ m_{1}\Delta_{1} }_{ \geq 0 }+\underbrace{ m_{2}\Delta_{2} }_{ \geq 0 }+\ldots+\underbrace{ m_{k}\Delta_{k} }_{ > 0 } + \ldots + \underbrace{ m_{\left| P \right| }\Delta_{\left| P \right| } }_{ \geq 0 }> 0
$$
