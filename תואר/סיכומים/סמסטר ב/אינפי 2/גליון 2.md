...
נגדיר חלוקה $Q=\left\{a,x_{0}-\delta,x_{0}+\delta,b \right\}$. כל נקודה $x$ בתת-קטע $\left[ a_{1},a_{2} \right]$  מקיימת $f\left( x \right)\geq \frac{f\left( x_{0} \right)}{2}$, לכן $m_{2}=\inf_{x \in \left[ a_{1},a_{2} \right]}f\left( x \right)\geq \frac{f\left( x_{0} \right)}{2}$
נסתכל על $L\left( Q,F \right)$
$$
	L\left( P,f \right) = \sum_{i}m_{i}\Delta_{i}= \underbrace{ m_{1}\Delta_{1} }_{ \geq 0 }+\underbrace{ m_{2}\Delta_{2} }_{ > 0 } + \underbrace{ m_{3}\Delta_{3} }_{ \geq 0 }> 0
$$
מצאנו חלוקה שעבורה $L\left( Q,f \right)>0$, מכיוון ש $L\left( Q,f \right)\leq\sup_{P}\left( P,f \right)$:
$$
