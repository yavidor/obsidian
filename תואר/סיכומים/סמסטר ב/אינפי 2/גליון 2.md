...
$$
מכיוון ש $f\geq 0$ אבל $f\not\equiv0$, קיימת נקודה כלשהי $x_{0}\in \left( a,b \right)$ (ידוע לנו כי קיימת נקודה כזו על $\left[ a,b \right]$, ומרציפות ומשפט ערך הביניים זה נותן כי קיימת סביבת של אותה הנקודה בה הפונקציה עדיין חיובית, לכן נוכל לבחור נקודה בתוך הקטע הפתוח)  בה $f\left( x_{0} \right)>0$. ממשפט ערך הביניים, קיים $\delta>0$ כך שלכל $x \in \left[ x_{0}-\delta,x_{0}+\delta \right]$ מתקיים $f\left( x \right)\geq \frac{f\left( x_{0} \right)}{2}$, נבחר את $\delta$ כך ש $\delta<\min\left\{ x_{0}-a,b-x_{0} \right\}$ כדי לוודא שנשאר בתחום $\left[ a,b \right]$.
