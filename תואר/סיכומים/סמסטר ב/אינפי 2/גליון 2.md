...
m_{i}	\underset{ x \in \left[ a_{i}-a_{i-1} \right]  }{ \inf } f\left( x \right)
	\end{gather}
$$
$f$ גזירה ב$\left[ a,b \right]$, לכן רציפה שם ולכן בכל קטע סגור $\left[ a_{i},a_{i-1} \right]$ היא מקבלת את המקסימום והמינימום שלה, ולכן
$$
	\begin{gather}
	...
	\end{gather}
$$
לכל $\left[ a_{i},a_{i-1} \right]$ קטע בחלוקה $P$, נסמן $\alpha_{i}\in \left[ a_{i},a_{i-1} \right]$ להיות הנקודה הראשונה בה מתקבל $M_{i}$, ו $\beta \in \left[ a_{i},a_{i-1} \right]$ להיות הנקודה האחרונה בה מתקבל $m_{i}$ (נשים לב ש-$\alpha_{i}\neq\beta_{i}$ שכן אפילו אם $m_{i}=M_{i}$, $\alpha_{i}=a_{i}$ ו$\beta=a_{i-1}$).
ממשפט לגרנז' ידוע לנו כי קיימת נקודה $c_{i}$ בין $\alpha_{i}$ ל-$\beta_{i}$ כך ש
$$
	f'\left( c \right)  = \frac{M_{i}-m_{i}}{\alpha_{i}-\beta_{i}}
$$